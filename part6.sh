...
echo "Migrate img_assist content filters to inline <img> tags: drush php-script ../all/migrate_scripts/img_assist_convert.php (run multiple times)"
...
echo "Remove duplicate fields created by the upgrade - term references, duplicate titles and 'taxonomy upgrade extras' -- reverting the features doesn't clean every single one of them out - see resource_evaluation nodes for examples"
echo "Disable conversion modules when all done: drush dis content_taxonomy_migrate field_convert image_legacy migrate content_dashboard AND ANY OTHERS"
